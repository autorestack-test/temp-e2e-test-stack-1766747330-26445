...
continue_after_resolution() {
    ...
    local NEW_TARGET
    NEW_TARGET=$(gh pr list --head "$OLD_BASE" --state merged --json baseRefName --jq '.[0].baseRefName' 2>/dev/null || echo "")

    if [[ -z "$NEW_TARGET" ]]; then
        echo "⚠️ Could not find where '$OLD_BASE' was merged to; skipping base branch update"
    else
    ...
    {
        echo -n "✅ Conflict resolved!"
        if [[ -n "$NEW_TARGET" ]]; then
            echo " Base branch updated to \`$NEW_TARGET\`."
        else
            echo ""
        fi
        echo "Continuing to update dependent PRs..."
